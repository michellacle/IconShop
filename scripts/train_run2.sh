#!/bin/bash
# First real training run - conservative settings
# 2 epochs, 2 workers/GPU, batch 4/GPU

set -e
cd "$(dirname "$0")/.."

# Increase NCCL timeout to prevent distributed hangs (default 30min is too short for large datasets)
